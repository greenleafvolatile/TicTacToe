...
/**
 * The TicTacToeFrame class builds a Tic Tac Toe game grid.
 */
public class TicTacToeFrame extends JFrame {
    ...
    class TicTacToeListener extends MouseAdapter {
        ...
        public void mousePressed(MouseEvent e){
            ...
            if(!tile.isFilled()){
                if(TicTacToeTile.isFirstMove()){
                    TicTacToeTile.startGame();
                }
                tile.draw();
                TicTacToeFrame.this.squaresFilled++;
                TicTacToeFrame.this.markOnGrid(tile.getRow(),tile.getColumn());
                TicTacToeFrame.this.checkEndGameConditions();
            }
        }
    }
    ...
    public void addTiles(){
        ...
        for(int i=0;i<NUMBER_OF_ROWS;i++){
            for(int j=0;j<NUMBER_OF_COLUMNS;j++){
                JComponent tile =new TicTacToeTile(i, j);
    ...
    public void markOnGrid(int row, int column){
        if(TicTacToeTile.isNoughts()){
            ticTacToeGrid[row][column]=NOUGHTS_VALUE;
        }
        else if(!TicTacToeTile.isNoughts()){
    ...
    public void checkEndGameConditions() {
        int gridValue= TicTacToeTile.isNoughts() ? NOUGHTS_VALUE : CROSSES_VALUE;
        ...
        if(gameWon || TicTacToeFrame.this.squaresFilled==NUMBER_OF_ROWS*NUMBER_OF_COLUMNS){
    ...
    public void displayEndOfGameMessage(){
        ...
        String title="";
        if(gameWon) {
            String winner = TicTacToeTile.isNoughts() ? "Noughts" : "Crosses";
